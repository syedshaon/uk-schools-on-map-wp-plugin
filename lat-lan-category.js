...
async function logAddressesWithCoordinates(addresses) {
  ...
  for (const item of addresses) {
    ...
    if (result && result.place_id) {
      const placeDetails = await getPlaceDetailsByPlaceId(result.place_id);

      addressWithCoords.push({
        address: item.address,
        latitude: result.geometry.location.lat(),
        longitude: result.geometry.location.lng(),
        category: placeDetails.types, // Assuming your address objects contain a "category"
      });
    }
  }

  console.log(addressWithCoords);
}
...
function geocodeAddress(address) {
  return new Promise((resolve, reject) => {
    geocoder.geocode({ address: address }, function (results, status) {
      if (status === "OK") {
        resolve(results[0]); // Return the lat/lng
      } else {
        console.error("Geocode failed for address:", address, "due to", status);
        resolve(null); // Resolve with null in case of failure
      }
    });
  });
}
...
async function initMap() {
  ...
  logAddressesWithCoordinates(addresses);
